        ...
        for j, col in enumerate(line):
            if col in symbols:
                ...
                if i - 1 >= 0:
                    ...
                    if (
                        j - 1 >= 0
                        and matrix[i - 1][j - 1].isdigit()
                        and check_up_corners
                    ):
                ...
                if i - 1 >= 0:
                    ...
                    if matrix[i - 1][j].isdigit():
                        ...
                        if j + 1 <= len(matrix[i - 1]):
                            value_up_right = run_through(j + 1, i - 1, matrix, "r")
                        part_nums.append(
                            int(value_up_left + matrix[i - 1][j] + value_up_right)
                        )
                        matrix[i - 1][j] = "."
                        check_up_corners = False
                ...
                if i + 1 <= len(matrix):
                    ...
                    if matrix[i + 1][j].isdigit():
                        ...
                        if j + 1 <= len(matrix[i + 1]):
                            value_down_right = run_through(j + 1, i + 1, matrix, "r")
                        part_nums.append(
                            int(value_down_left + value_down + value_down_right)
                        )
                        check_down_corners = False
...
def run_through(
    start: int, line_index: int, matrix: list[list[str]], direction: str = ""
) -> str:
